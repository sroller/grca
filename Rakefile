...
namespace :deploy do
  ...
  task :nginx_config do
    ...
    unless File.exist?(nginx_config)
      abort "Error: nginx.example.conf not found in project root"
    end
  ...
  task :systemd_service do
    ...
    service_content = <<~SERVICE
      [Unit]
      Description=GRCA Web Application
      ...
      ExecStart=/usr/bin/bundle exec ruby bin/grca_web -p 4567
      Restart=always
      RestartSec=5
  ...
  task :all => [:install_gems, :copy_files, :systemd_service, :nginx_config] do
    puts "\n" + "="*60
